...
class mmr_summarize(object):
    ...
    ''' Function to get the single Best matching sentence '''
  	## Find best sentences out of all the the sentences by using cosine similarity
        ...
        for i in range(summary_length):
            ...
            for sent in sentences:
                ...
                if curr > prev:
                    prev = curr
                    best_line = sent
                    #if(tb(best_line.getOGwords()).sentiment.polarity >=0):
                     #   print (best_line.getOGwords())
                     #   continue  

            selected_sentences += [best_line]
            sentences.remove(best_line)
            ##print selected_sentences.getOGwords()
        ##******************************************************* Polarity ****************************************
        ## comment  on uncomment following loop to disable/enable poalrity support in summary
        ## if polarity <0  -> negative sentences will be removed (positive toning)
        ## if polarity >0  -> positive sentence will be removed  (negative toning)
	## introducing polarity can decrease the Rouge score of the document summary
	
        '''
        for x in selected_sentences:
            if(tb(x.getOGwords()).sentiment.polarity <0):
                selected_sentences.remove(x)
        '''
	...
	## This particular part implements the actual proposed MMR formula by researchers	
	...
	#select best sentences to be sent ro MMR 
        best_sentence = self.getBestSentence(sentences, query, idfs)
	# call to MMR for creating summary
